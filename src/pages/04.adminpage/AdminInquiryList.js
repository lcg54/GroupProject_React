    ...
    try {
      await axios.post(
        `${API_BASE_URL}/product/${
          inquiries.find((i) => i.id === inquiryId).productId
        }/inquiry/${inquiryId}/comment`,
        {
          adminId: user.id,
          comment: comment,
        }
      ...
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        <div className="d-flex gap-2">
          <Dropdown>
            ...
            <Dropdown.Menu>
              <Dropdown.Item onClick={() => setFilterAnswered(null)}>
                전체
              </Dropdown.Item>
              <Dropdown.Item onClick={() => setFilterAnswered(false)}>
                답변 대기
              </Dropdown.Item>
              <Dropdown.Item onClick={() => setFilterAnswered(true)}>
                답변 완료
              </Dropdown.Item>
            </Dropdown.Menu>
          </Dropdown>
              ...
              <Accordion.Item eventKey={String(idx)} key={inquiry.id}>
                <Accordion.Header>
                  <div className="d-flex align-items-center gap-2 w-100">
                    ...
                    <Badge
                      bg={inquiry.adminComment ? "success" : "danger"}
                      className="ms-2"
                    >
                ...
                <Accordion.Body>
                  ...
                  {inquiry.adminComment && (
                    <Card bg="light" className="p-3 mt-4">
                      <div className="d-flex justify-content-between text-muted mb-2">
                        <span>
                          고객센터 (담당자: {inquiry.adminComment.admin})
                        </span>
                        <span>
                          {new Date(
                            inquiry.adminComment.createdAt
                          ).toLocaleString()}
                        </span>
                      </div>
                        ...
                        <>
                          ...
                          <div className="d-flex justify-content-end gap-2 mt-2">
                            <Button
                              ...
                              size="sm"
                              onClick={() =>
                                toggleEditComment(inquiry.id, null)
                              }
                            >
                              취소
                            </Button>
          ...
          {[...Array(Math.min(10, paging.totalPages))].map((_, idx) => {
            ...
            return (
              <Pagination.Item
                ...
                onClick={() =>
                  setPaging((prev) => ({ ...prev, pageNumber: pageNum }))
                }
              >
                {pageNum + 1}
              </Pagination.Item>
